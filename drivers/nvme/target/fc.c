...
static void nvmet_fc_delete_assoc_work(struct work_struct *work)
{
	struct nvmet_fc_tgt_assoc *assoc =
...
static void nvmet_fc_schedule_delete_assoc(struct nvmet_fc_tgt_assoc *assoc)
{
	int terminating;
...
static bool nvmet_fc_assoc_exists(struct nvmet_fc_tgtport *tgtport,
				  u64 association_id)
{
...
static void nvmet_fc_free_pending_reqs(struct nvmet_fc_tgtport *tgtport)
{
	struct nvmet_fc_ls_req_op *lsop;
	struct nvmefc_ls_req *lsreq;
	struct nvmet_fc_ls_iod *iod;
...
static ssize_t nvmet_fc_host_traddr(struct nvmet_ctrl *ctrl, char *traddr,
				    size_t traddr_size)
{
	...
	struct nvmet_fc_tgtport *tgtport =
		queue->assoc ? queue->assoc->tgtport : NULL;
	struct nvmet_fc_hostport *hostport =
		queue->assoc ? queue->assoc->hostport : NULL;
	u64 wwnn, wwpn;
	ssize_t ret = 0;
	...
	if (tgtport->ops->host_traddr) {
		ret = tgtport->ops->host_traddr(hostport->hosthandle, &wwnn,
						&wwpn);
		if (ret)
			goto out_put_host;
		ret = snprintf(traddr, traddr_size, "nn-0x%llx:pn-0x%llx", wwnn,
			       wwpn);
	}
out_put_host:
